...
//
// Author:   SungWoo Kim - Completed implementation.
//
   ...
   for (int step = 1; step <= num_steps; step++)
   {
      ...
      // Finding pairs (R)
       for (int r = 0; r < SIZE; r++)
          for (int c = 0; c < SIZE; c++)
          {
            if ((board [r][c] == 'R') && (c+1 < SIZE) && (board [r][c+1] == 'R'))
                {
                    cout << "Found pair(R) " << r << " " << c << endl; 
                      // Reproduce Rabbit
                      int new_r = random() % SIZE;
                      int new_c = random() % SIZE;
                      if (board[new_r][new_c] == ' ')
                      {
                         board[new_r][new_c] = 'R';
                         cout << "New Rabbit " << new_r << " " << new_c << endl; 
                      }
                }
          }
       ...
       for (int r = 0; r < SIZE; r++)
          for (int c = 0; c < SIZE; c++)
          {
            if ((board [r][c] == 'F') && (r+1 < SIZE) && (board [r+1][c] == 'F'))
                {
                    cout << "Found pair(F) " << r << " " << c << endl; 
                      // Reproduce Fox
                      int new_r = (random() % SIZE);
                      int new_c = (random() % SIZE);
                      if ((board[new_r][new_c] == ' ') && (random() % 2 == 0))
                      {
                         board[new_r][new_c] = 'F';
                         cout << "New Fox " << new_r << " " << new_c << endl; 
                      }
                }
          }
         ...
         for (int r = 0; r < SIZE; r++)
            for (int c = 0; c < SIZE; c++)
            {
               if ((board [r][c] == 'R')  && (board [r][c+1] == 'F') && (c+1 < SIZE)) 
                    {
                        board [r][c] = ' ';
                         cout << "Rabbit's got eaten " << r << " " << c << endl;
                    }
               else if ((board [r][c] == 'R')  && (board [r][c-1] == 'F') && (c-1 < SIZE))     
                    {
                        board [r][c] = ' ';
                         cout << "Rabbit's got eaten " << r << " " << c << endl;
                    }
               else if ((board [r][c] == 'R')  && (board [r+1][c] == 'F') && (r+1 < SIZE))     
                    {
                        board [r][c] = ' ';
                         cout << "Rabbit's got eaten " << r << " " << c << endl;
                    }
